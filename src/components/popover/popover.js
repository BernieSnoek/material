...
MdPopoverService.prototype.create = function(scope, element, attrs) {
  ...
  var popoverRef = new MdPopoverRef(this._$injector, this._positions, scope, this._config);
  ...
  if (scope.mdEnabled) {
    ...
    scope.$on('$destroy', angular.bind(popoverRef, popoverRef._destroy));
    popoverRef._config.element.one('$destroy', angular.bind(popoverRef, popoverRef._destroy));
    popoverRef._config.parentElement.one('$destroy', angular.bind(popoverRef, popoverRef._destroy));
  }
 ...
 * @param {!Object} config
 */
function MdPopoverRef($injector, positions, scope, config) {
  // Injected variables.
...
MdPopoverRef.prototype._updatePosition = function() {
  ...
  this._config.positionClass = 'md-position-' + this._scope.mdPosition;
  this._config.position = this._positions[this._scope.mdPosition];
...
MdPopoverRef.prototype._configureVisiblePositionWatcher = function() {
  ...
  this._visibleWatcher = this._scope.$watch('mdVisible', visibleHandlerFn);
  this._positionWatcher = this._scope.$watch('mdPosition', positionHandlerFn);
};
...
MdPopoverRef.prototype._configureAriaLabelWatcher = function() {
  ...
  this._ariaLabelWatcher = this._scope.$watch(function() {
...
MdPopoverRef.prototype._configureOpenTriggers = function() {
  ...
  var handlerFn = function(ev) {
    ...
    } else if (!self._scope.mdVisible) {
      self._updatePosition();
      self._setVisible(true);
    }
  };

  this._config.parentElement.on(this._scope.mdOpenTrigger, handlerFn);

  this._removeListeners.push(function() {
    self._config.parentElement.off(self._scope.mdOpenTrigger, handlerFn);
  });
};
...
MdPopoverRef.prototype._configureCloseTriggers = function() {
  ...
  var handlerFn = function() {
    self._autohide = self._scope.hasOwnProperty('mdAutohide') ?
        self._scope.mdAutohide :
  ...
  this._config.parentElement.on(this._scope.mdCloseTrigger, handlerFn);

  this._removeListeners.push(function() {
    self._config.parentElement.off(self._scope.mdCloseTrigger, handlerFn);
  });
};
...
MdPopoverRef.prototype._setVisible = function(isVisible) {
  ...
  var openHandlerFn = function() {
    self._scope.mdVisible = self._setVisible.value;
    self._setVisible.queued = false;
    self._openTimeout = null;
    if (!self._visibleWatcher) {
      self._onVisibleChanged(self._scope.mdVisible);
    }
  };
  ...
  if (this._setVisible.queued && this._setVisible.value === !!isVisible ||
      !this._setVisible.queued && this._scope.mdVisible === !!isVisible) {
    return;
  }
  ...
  if (!this._setVisible.queued) {
    if (isVisible) {
      ...
      this._openTimeout = this._$timeout(openHandlerFn, self._scope.mdOpenDelay);
    } else {
      this._closeTimeout = this._$timeout(closeHandlerFn, self._scope.mdCloseDelay);
    }
  }
};
...
MdPopoverRef.prototype._open = function() {
  ...
  panelClass += this._scope.mdPopoverClass ? ' ' + this._scope.mdPopoverClass : '';
...
MdPopoverRef.prototype._addAriaLabelToParent = function(override) {
  ...
  if (!this._config.isTooltip && !this._config.parentElement.attr('aria-label')) {
    ...
    var ariaLabel = this._config.isTooltip ? this._config.element[0].textContent.trim() : null;
    var rawText = override || ariaLabel;
    var interpolatedText = this._$interpolate(rawText)(this._scope);
